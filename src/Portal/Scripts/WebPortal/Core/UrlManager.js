Microsoft.WebPortal.Core.UrlManager = function (webPortal) {
    ...
    this.webPortal.EventSystem.subscribe(Microsoft.WebPortal.Event.WidgetActivated, this.onWidgetActivated, this);
...
Microsoft.WebPortal.Core.UrlManager.prototype.onPortalInitialized = function () {
    ...
    if (window.location.hash && window.location.hash.length > 0) {
        ...
        var featureList = window.location.hash.slice(1).split("/");
        var idx = 1;
        var json = "{";
...
Microsoft.WebPortal.Core.UrlManager.prototype.onFeatureActivated = function (event, featureInformation) {
    ...
    if (featureInformation.context) {
        ...
        $.each(featureInformation.context, function (key, value) {
            hashUrlPath += "/" + key + "/" + value;
        });
    }
...
Microsoft.WebPortal.Core.UrlManager.prototype.onWidgetActivated = function (event, featureInformation) {
    /// <summary>
    /// Invoked whenever a feature gets activated.
    /// </summary>
    /// <param name="event">The event Id.</param>
